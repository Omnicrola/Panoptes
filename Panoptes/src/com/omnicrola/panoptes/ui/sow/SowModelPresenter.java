...
public class SowModelPresenter {
	...
	public static final WorkStatement NEW = new WorkStatement("<NEW>", "", "", ProjectGroup.CLIENT_BILLABLE);
	...
	public void setCurrentWorkStatement(WorkStatement workStatement) {
		...
		for (ISowViewUpdater updater : this.viewList) {
			updater.currentSelectionChanged(projectName, projectCode, client, group);
		}
	}

	public void updateCurrentWorkOrder(String projectName, String client, String projectCode, ProjectGroup projectGroup) {
		if (this.currentWorkStatement == NEW) {
			WorkStatement workStatement = new WorkStatement(projectName, client, projectCode, projectGroup);
			this.controller.addWorkStatement(workStatement);
			notifyViewsOfNewStatement(workStatement);
			setCurrentWorkStatement(NEW);
		} else {
			this.currentWorkStatement.setData(projectName, client, projectCode, projectGroup);
		}
	}
