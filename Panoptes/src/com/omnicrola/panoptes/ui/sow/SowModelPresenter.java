...
public class SowModelPresenter {
    ...
    public static final WorkStatement NEW = new WorkStatement("<NEW>", "", "", "", 0);
    ...
    public void setCurrentWorkStatement(WorkStatement workStatement) {
        ...
        for (ISowViewUpdater updater : this.viewList) {
            updater.currentSelectionChanged(projectName, projectCode, client, sowCode, billableRate);
        }
    }

    public void updateCurrentWorkOrder(String projectName, String client, String projectCode,
            String sowCode, float billableRate) {
        if (this.currentWorkStatement == NEW) {
            WorkStatement workStatement = new WorkStatement(projectName, client, projectCode,
                    sowCode, billableRate);
            this.controller.addWorkStatement(workStatement);
            notifyViewsOfNewStatement(workStatement);
            setCurrentWorkStatement(NEW);
        } else {
            this.currentWorkStatement.setData(projectName, client, projectCode, sowCode,
                    billableRate);
        }
    }
